...
void lcd_set_xy(uint8_t x,uint8_t ypage);
...
{
	int16_t  dx,sx,dy,sy,err,e2;  // for Bresenham
	uint8_t	i;
...
// Return:    -
//--------------------------------------------------------------------------------------------------
void LCD_PrintXY(uint8_t x0,uint8_t y0,char *s)
{
	uint8_t	ix,iy,y;
	const uint8_t *font;
	const uint8_t *pt;
	uint8_t 	d;
	char c;
	uint8_t char_width,char_height,char_size;
	uint8_t i_char_height;
